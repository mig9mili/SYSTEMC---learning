...
SC_MODULE (Source) {
  sc_fifo_out<SysPE::InputType> act_in_vec[N];
  sc_fifo_out<SysPE::InputType> weight_in_vec[N];
  sc_in <bool> clk;
  sc_in <bool> rst;
  ...
  void Run() {
    for (int i = 0; i < N; i++) {
      act_in_vec[i].write(0);
      weight_in_vec[i].write(0);
    }
    ...
    for (int j = N-1; j >= 0; j--) {
      for (int i = 0; i < N; i++) {
        weight_in_vec[i].write(W_mat[i][j]);
      }
    } 
...
SC_MODULE (Sink) {
  sc_fifo_in<SysPE::AccumType> accum_out_vec[N];
  sc_in <bool> clk;
  sc_in <bool> rst;
  ...
  void Run() {
    ...
    while (1) {
      ...
      for (int i = 0; i < N; i++) {
        ...
        is_out[i] = accum_out_vec[i].nb_read(_out);
        if (is_out[i]) {
          out_vec[i] = _out;
...
SC_MODULE (testbench) {
  ...
  sc_fifo<SysPE::InputType> act_in_vec[N];
  sc_fifo<SysPE::InputType> weight_in_vec[N];
  sc_fifo<SysPE::AccumType> accum_out_vec[N];
